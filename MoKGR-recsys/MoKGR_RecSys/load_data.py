"""
DataLoader for MoKGR on RecSys datasets.
Follows AdaProp RecSys data loading (user-item CF + KG unified graph),
with optional PPR subgraph sampling from original MoKGR.
"""
...
class DataLoader:
    ...
    # ------------------------------------------------------------------
    def _init_ppr(self):
        homo_edges = [(h, t) for h, r, t in self.fact_data]
        self.ppr_sampler = PPRSampler(
            self.n_nodes, homo_edges,
    ...
    def load_graph(self, triples):
        ...
        Self-loop relation id = 2 * n_rel + 2.
        """
    ...
    def get_neighbors(self, nodes, mode='train'):
        """Return one-hop neighbors of *nodes*.
        ...
        node_1hot = csr_matrix(
            ...
            shape=(self.n_nodes, max_batch_idx + 1),
        )
