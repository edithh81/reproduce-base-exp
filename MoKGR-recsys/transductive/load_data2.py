...
class DataLoader2:
    ...
    def load_query(self, triples):
        ...
        for trip in triples:
            ...
            max_answers = max(max_answers, len(trip_hr[(h, r)]))

        queries = []
        answers = []
        for key in trip_hr:
            queries.append(key)
            current_answers = np.array(trip_hr[key])
            padded_answers = np.zeros(max_answers, dtype=np.int64)
            padded_answers[:len(current_answers)] = current_answers
            answers.append(padded_answers)
    ...
    def get_neighbors(self, nodes,mode='train'):
        ...
        # maybe the information of hop should be put together as well
    ...
    def get_batch(self, batch_idx, data='train'):
        ...
        for i in range(len(batch_idx)):
            ...
            valid_answers = answer[batch_idx[i]][answer[batch_idx[i]] != 0]
            objs[i][valid_answers] = 1
    ...
    def shuffle_train(self):
        ...
        if self.args.remove_1hop_edges:
            ...
            tmp_index[self.fact_data[:, 0], self.fact_data[:, 2]] = 0
