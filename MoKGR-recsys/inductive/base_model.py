...
class BaseModel(object):
    ...
    def train_batch(self):
        ...
        for i in tqdm(range(n_batch), desc="Training Epoch Progress", unit="batch"):
            ...
            scores, G_full, Q = self.model(triple[:,0], triple[:,1])
            ...
            total_loss = loss + self.lambda_importance * L_importance + self.lambda_load * L_load
    ...
    def evaluate(self):
        ...
        self.model.eval()
        time_3 = time.time()
        for i in tqdm(range(n_batch), desc="Validation Progress", unit="batch"):
            start = i * batch_size
            end = min(n_data, (i+1)*batch_size)
            batch_idx = np.arange(start, end)
            subs, rels, objs = self.loader.get_batch(batch_idx, data='valid')
            scores = self.model(subs, rels, 'transductive').data.cpu().numpy()  # 使用 'transductive' 模式
            filters = []
            for j in range(len(subs)):
                filt = self.loader.val_filters[(subs[j], rels[j])]
                filt_1hot = np.zeros((self.n_ent, ))  # 使用 self.n_ent
                filt_1hot[np.array(filt)] = 1
                filters.append(filt_1hot)
                masks += [self.n_ent - len(filt)] * int(objs[j].sum())
        ...
        masks = []
        self.model.eval()
        for i in tqdm(range(n_batch), desc="Test Progress", unit="batch"):
            start = i*batch_size
            end = min(n_data, (i+1)*batch_size)
            batch_idx = np.arange(start, end)
            subs, rels, objs = self.loader.get_batch(batch_idx, data='test')
            scores = self.model(subs, rels, mode='inductive').data.cpu().numpy()
            filters = []
            for j in range(len(subs)):
                filt = self.loader.tst_filters[(subs[j], rels[j])]
                filt_1hot = np.zeros((self.n_ent_ind, ))  # using n_ent_ind
                filt_1hot[np.array(filt)] = 1
                filters.append(filt_1hot)
                masks += [self.n_ent_ind - len(filt)] * int(objs[j].sum())
            filters = np.array(filters)
            ranks = cal_ranks(scores, objs, filters)
            ranking += ranks
        ranking = np.array(ranking)
        ...
        out_str = f'[Epoch {self.current_epoch}] Valid MRR: {v_mrr:.4f}, MR: {v_mr:.1f}, H@1: {v_h1:.4f}, H@3: {v_h3:.4f}, H@10: {v_h10:.4f}, H@10@50: {v_h1050:.4f} \t Test MRR: {t_mrr:.4f}, MR: {t_mr:.1f}, H@1: {t_h1:.4f}, H@3: {t_h3:.4f}, H@10: {t_h10:.4f}, H@10@50: {t_h1050:.4f}\n'
